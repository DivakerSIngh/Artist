...
$('.item-img').on('change', function () {
	...
	//logic for video upload in community section
	var typeOfFile = tempFilename.split('.').pop();
	if (typeOfFile.toUpperCase() == "mp4".toUpperCase()) {
		var divModalComunity = angular.element(document.getElementById('divModalComunity')).scope();
		if (divModalComunity) {
			divModalComunity.$apply(function () {
				divModalComunity.communityImageUplaod($('input[name=file_photo]'), 'imgUploadvideo')
			});
		}
	} else {
...
$('#cropImageBtn').on('click', function (ev) {
	$uploadCrop.croppie('result', {
		...
		if (divModalComunity) {
			divModalComunity.$apply(function () {
				divModalComunity.uploadCropedImageForComunity(blob);
			});
		}
